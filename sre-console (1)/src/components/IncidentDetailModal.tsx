      ...
      <div className="bg-[#0a0a0a] border border-[#ffffff1a] rounded-md w-full max-w-3xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="p-5 border-b border-[#ffffff1a] bg-[#141414] flex justify-between items-center">
          <div className="flex items-center gap-3">
            <span
              className={`px-2.5 py-1 rounded text-sm font-bold font-code-sm ${
            ...
            <div>
              ...
              <p className="font-code-sm text-sm text-[#a3a3a3]">{incident.timestamp}</p>
            </div>
          </div>
        ...
        <div className="p-6 space-y-5 overflow-y-auto font-sans text-sm">
          {/* Summary */}
          <div>
            <h3 className="font-label-caps text-sm text-[#d4af37] mb-1 tracking-wider uppercase font-semibold">Incident Summary</h3>
          ...
          <div className="bg-[#141414] p-4 rounded border border-[#ffffff1a]">
            <h3 className="font-label-caps text-sm text-[#ef4444] mb-1.5 flex items-center gap-1 tracking-wider uppercase font-semibold">
              <span className="material-symbols-outlined text-[16px]">psychology</span> Root Cause Analysis
            </h3>
            <p className="text-[#a3a3a3] font-code-sm text-sm leading-relaxed font-light">
          ...
          {incident.mitigationSteps && (
            <div>
              <h3 className="font-label-caps text-sm text-[#a3a3a3] mb-2 tracking-wider uppercase font-semibold">Remediation Executed</h3>
              <ul className="list-disc list-inside font-code-sm text-sm text-[#f5f5f5] space-y-1 font-light">
          ...
          {incident.metricsSnapshot && (
            <div className="grid grid-cols-3 gap-3 bg-[#141414] p-4 rounded border border-[#ffffff1a] font-code-sm text-sm text-center">
              <div>
                <div className="text-[#a3a3a3] text-[12px] uppercase tracking-wider">Peak RPS</div>
                <div className="text-[#f5f5f5] font-bold mt-1 text-sm">{incident.metricsSnapshot.rps}</div>
              </div>
              <div>
                <div className="text-[#a3a3a3] text-[12px] uppercase tracking-wider">Error Rate</div>
                <div className="text-[#ef4444] font-bold mt-1 text-sm">{incident.metricsSnapshot.errorRate}%</div>
              </div>
              <div>
                <div className="text-[#a3a3a3] text-[12px] uppercase tracking-wider">P99 Latency</div>
          ...
          <div className="pt-3 border-t border-[#ffffff1a] space-y-3">
            <label className="font-label-caps text-sm text-[#d4af37] flex items-center gap-1 tracking-wider uppercase font-semibold">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="input-tactical flex-1 bg-[#141414] border border-[#ffffff1a] rounded p-2.5 text-sm font-code-sm text-[#f5f5f5] placeholder-[#a3a3a3]"
              />
              <button
                onClick={handleAskAi}
                disabled={isAsking}
                className="px-5 py-2.5 bg-[#d4af37] hover:bg-[#e2bd46] text-[#0a0a0a] font-bold rounded font-label-caps text-sm cursor-pointer shadow-md shadow-[#d4af371a] uppercase tracking-wider"
              >
                {isAsking ? 'Asking...' : 'Ask'}
              </button>
            </div>
            {aiResponse && (
              <div className="p-3.5 bg-[#d4af370f] border border-[#d4af3766] rounded font-code-sm text-sm text-[#f5f5f5] font-light">
                {aiResponse}
              </div>
            )}
          </div>
        ...
        <div className="p-4 border-t border-[#ffffff1a] bg-[#141414] flex justify-end items-center">
          <button
            onClick={onClose}
            className="px-5 py-2 bg-[#d4af37] hover:bg-[#e2bd46] text-[#0a0a0a] font-bold font-label-caps text-sm rounded cursor-pointer shadow-md shadow-[#d4af371a] uppercase tracking-wider"
          >
            Done
          </button>
        </div>
