      ...
      <form onSubmit={handleSearch} className="bg-[#101317] border border-[#EDE7DC]/13 rounded p-6 space-y-4">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="w-full md:w-48 flex items-center gap-3">
            <span className="text-xs font-mono text-[#9EA5A8]">TOP K: {topK}</span>
            <input
              type="range"
              min="1"
              max="10"
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="btn-sre-outline btn-sre-teal px-6 py-3 text-xs flex items-center gap-2"
          >
            ...
            >
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[#EDE7DC]/10 pb-4">
                <div className="flex items-center gap-3">
                  <span className="text-xs font-mono px-2 py-1 bg-[#0A0C0E] border border-[#EDE7DC]/10 rounded text-[#2E6B72] uppercase font-bold">
                    {docType}
                  </span>
                  <span className="text-xs font-mono text-[#9EA5A8]">{doc.id}</span>
                  {isTopResult && (
                    <span className="text-[10px] font-mono text-[#E8913C] border border-[#E8913C]/40 px-2 py-0.5 rounded uppercase">
                      TOP MATCH
                    </span>
                  )}
                </div>
                <div className="text-xs font-mono text-[#2E6B72] font-bold">
                  RELEVANCE SCORE: {doc.score.toFixed(4)}
                </div>
              </div>

              <h3 className="font-display text-lg text-[#EDE7DC] mt-4 mb-2">{doc.title}</h3>
              <p className="text-xs text-[#9EA5A8] font-mono leading-relaxed bg-[#0A0C0E] p-4 rounded border border-[#EDE7DC]/10">
                {doc.text}
              </p>
              ...
              {doc.tags && (
                <div className="flex flex-wrap gap-2 mt-4">
                  {(Array.isArray(doc.tags) ? doc.tags : String(doc.tags).split(',')).map((tag) => (
                    <span key={tag} className="text-[10px] font-mono text-[#6C7378] bg-[#0A0C0E] px-2 py-1 rounded border border-[#EDE7DC]/10">
                      #{tag.trim()}
                    </span>
                  ))}
                </div>
