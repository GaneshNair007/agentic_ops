  ...
  return (
    <section ref={containerRef} className="relative w-full h-[220vh] bg-[#050505] text-[#F1F1F1]">
      ...
      <div ref={heroStageRef} className="w-full h-screen relative overflow-hidden flex flex-col justify-between p-6 md:p-12">
        ...
        {/* Full-Colour Server Infrastructure Photograph (Expands from 35vw -> 100vw) */}
        <div 
          ref={imageStripRef} 
          className="absolute top-0 bottom-0 left-1/2 -translate-x-1/2 w-[35vw] overflow-hidden z-0 shadow-2xl"
        >
          <img
            src="https://images.unsplash.com/photo-1558494949-ef010cbdcc31?auto=format&fit=crop&w=2000&q=90"
            alt="Full Color Server Rack Infrastructure"
        ...
        <div className="relative z-10 flex justify-between items-center text-xs font-mono text-[#C8C8C8] uppercase tracking-widest border-b border-white/20 pb-4">
        ...
        <div className="relative z-10 text-center select-none my-auto">
          <h1 className="font-display text-7xl md:text-9xl font-extrabold text-[#FFFFFF] tracking-tighter flex items-center justify-center gap-8 leading-none">
            ...
            <span ref={titleRightRef} className="inline-block">SRE</span>
          </h1>
        </div>

        {/* Bottom Hero Narrative & CTAs (Max width 58ch, 19px body) */}
        <div className="relative z-10 grid grid-cols-1 md:grid-cols-12 gap-6 items-end border-t border-white/20 pt-6">
          <div className="md:col-span-8 space-y-3">
            <div className="label-caps text-[#C8C8C8]">// INCIDENT TRIAGE & RESPONSE SYSTEM</div>
            <p className="prose-editorial text-lg md:text-xl text-[#F1F1F1] font-sans font-normal">
